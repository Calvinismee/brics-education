...
export function DashboardNotificationBell({
  ...
  markAllHref = null,
  allHref = null,
  historyLabel = "Riwayat terbaru",
  reloadProps = ["dashboardNotifications"],
}) {
  const { props } = usePage();
  const notificationData = shared ?? props.dashboardNotifications ?? {};
  const [open, setOpen] = useState(false);
  const [localUnread, setLocalUnread] = useState(notificationData.unreadCount ?? 0);
  const notifications = asArray(notificationData.latest);

  useEffect(() => {
    setLocalUnread(notificationData.unreadCount ?? 0);
  ...
  const openNotifications = () => {
    ...
    if (nextOpen && localUnread > 0 && markAllHref) {
      setLocalUnread(0);
      router.post(markAllHref, {}, {
        preserveScroll: true,
        preserveState: true,
        only: reloadProps,
      });
        ...
        <div className="absolute right-0 top-12 z-50 w-[calc(100vw-2rem)] max-w-[20rem] overflow-hidden rounded-2xl border border-[#D8D7BE] bg-white shadow-xl sm:top-14">
          <div className="px-4 py-3 border-b border-[#F7F2E7] flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400">{historyLabel}</p>
            </div>
